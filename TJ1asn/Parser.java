...
public final class Parser {
  ...
  private static void methodDecl() throws SourceFileErrorException
  {
    ...
    accept(STATIC);
      switch (getCurrentToken()) {
      	...
      	default: throw new SourceFileErrorException("Expected first token of a void or int, not "
  ...
  private static void assignmentOrInvoc() throws SourceFileErrorException
  {
    ...
    switch(getCurrentToken())
    {
    	...
    	default: throw new SourceFileErrorException("Expected first token of a <assignmentOrInvoc>, not"
                              + getCurrentToken().symbolRepresentationForOutputFile);	
    }
  ...
  private static void printArgument() throws SourceFileErrorException
  {
    ...
    switch (getCurrentToken()) {
      case CHARSTRING: nextToken(); break;
      case LPAREN: 
      ...
      default: throw new SourceFileErrorException("Expected first token of a <printArgument>, not "
                              + getCurrentToken().symbolRepresentationForOutputFile);
    }
  ...
  private static void expr1() throws SourceFileErrorException
  {
    ...
    switch (getCurrentToken()) {
      ...
      case NOT: nextToken(); 
      	  		 expr1();
      	  		 break;
      case UNSIGNEDINT: nextToken(); break;
      case NULL: nextToken(); break;
      case NEW: nextToken(); 
      	  		accept(INT);
      	  		accept(LBRACKET);
      	  		expr3();
      ...
      case IDENT: nextToken();
      	  		   ...
      	  		   else if(getCurrentToken() == LBRACKET)
      	  		   {
      	  		   	   while(getCurrentToken() == LBRACKET)
      	  		   	   {
      	  		   	   	   nextToken();
